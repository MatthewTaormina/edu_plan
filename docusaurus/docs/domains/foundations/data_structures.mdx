...
description: Arrays, linked lists, stacks, queues, deques, hash tables, trees, heaps, and graphs — with explanations and implementations in Python, TypeScript, Rust, and C.
...
- [ ] Explain the difference between a queue, a deque, and a heap — and when to use each
- [ ] Describe how a heap enforces its property and what problem it solves efficiently
- [ ] Choose the right data structure for a given problem
- [ ] State the time complexity of insert/delete/search for each structure
...
| Add/remove from **both** ends efficiently | Deque | Doubly-ended — O(1) at front and back |
| Always access the smallest (or largest) item | Heap / Priority Queue | Tree-shaped; root is always min/max |
| Look up items by a key instantly | Hash Table | Hash function → direct bucket |
| Search hierarchical relationships | Tree / BST | Divide and conquer by structure |
...
<img src="/img/diagrams/array_memory.svg" alt="Array Memory" style={{width:'100%',maxWidth:'720px',display:'block',margin:'1rem auto'}} />
...
<div className="svg-graphic-container margin-bottom--lg">
  <img src="/img/diagrams/stack_states.svg" alt="Stack States" style={{width:'100%',maxWidth:'720px',display:'block',margin:'1rem auto'}} />
</div>
...
<div className="svg-graphic-container margin-bottom--lg">
  <img src="/img/diagrams/queue_fifo.svg" alt="Queue Fifo" style={{width:'100%',maxWidth:'720px',display:'block',margin:'1rem auto'}} />
</div>
...
A **deque** ("deck") stands for **double-ended queue**. Unlike a basic queue (add back, remove front) or a stack (add/remove top only), a deque lets you **add or remove from either end in O(1)**.
...
**Real uses:** sliding window problems, palindrome checking, browser history (forward *and* back), work-stealing schedulers, implementing both stacks and queues from a single structure.

**How it works in code:** Internally, a deque is usually a **doubly-linked list** or a **ring buffer** (circular array). Both allow O(1) access at either end. A plain array/list does NOT: `unshift()` / `shift()` in JS, or `insert(0, x)` / `pop(0)` in Python, are O(n). Python's `collections.deque` is a doubly-linked list of fixed-size blocks — true O(1) at both ends.
...
| pop_front / pop_back | O(1) | Remove from either end |
| peek_front / peek_back | O(1) | Read without removing |
| Access by index | O(n) | Not its purpose — use an array for that |
...
CLASS Deque<T>
    ...
    FUNCTION push_front(value: T) -> Void
        prepend(self.data, value)     // O(1)
    END FUNCTION

    FUNCTION push_back(value: T) -> Void
        append(self.data, value)      // O(1)
    END FUNCTION

    FUNCTION pop_front() -> T
        IF self.is_empty() THEN THROW UnderflowError
        RETURN remove_front(self.data)
    END FUNCTION

    FUNCTION pop_back() -> T
        IF self.is_empty() THEN THROW UnderflowError
        RETURN remove_back(self.data)
    END FUNCTION
...
// Classic application: palindrome check
FUNCTION is_palindrome(s: String) -> Bool
    dq ← NEW Deque<Char>
    FOREACH char IN s DO push_back(dq, char) END FOREACH

    WHILE dq.size() > 1 DO
        IF pop_front(dq) != pop_back(dq) THEN RETURN FALSE
    END WHILE
    RETURN TRUE
END FUNCTION
```
...
# Python's deque IS the double-ended queue — use it directly
dq = deque()

dq.append(20)        # push_back  → [20]
dq.appendleft(10)    # push_front → [10, 20]
dq.append(30)        # push_back  → [10, 20, 30]

print(dq.popleft())  # pop_front → 10, deque: [20, 30]
print(dq.pop())      # pop_back  → 30, deque: [20]
...
# Sliding window maximum — a classic deque pattern
def sliding_max(nums: list[int], k: int) -> list[int]:
    dq, result = deque(), []
    for i, n in enumerate(nums):
        while dq and nums[dq[-1]] < n:
            dq.pop()          # discard smaller values at the back
        dq.append(i)
        if dq[0] == i - k:   # front index fell outside window
            dq.popleft()
        if i >= k - 1:
...
```typescript
// TypeScript has no built-in deque.
// For production O(1) front ops, install the 'denque' npm package (ring buffer).
// The version below wraps an array — front ops are O(n), fine for learning:
class Deque<T> {
    private data: T[] = [];
    pushBack(value: T): void   { this.data.push(value); }
    pushFront(value: T): void  { this.data.unshift(value); } // O(n)
    popBack(): T   { return this.data.pop()!; }
    popFront(): T  { return this.data.shift()!; }            // O(n)
    peekFront(): T { return this.data[0]; }
    peekBack(): T  { return this.data[this.data.length - 1]; }
    isEmpty(): boolean { return this.data.length === 0; }
    size(): number { return this.data.length; }
}
...
function isPalindrome(s: string): boolean {
    ...
    for (const ch of s) dq.pushBack(ch);
    while (dq.size() > 1) {
...
println!("{:?}", dq.pop_front());  // Some(10)
println!("{:?}", dq.pop_back());   // Some(30)

// Rust's VecDeque is a ring buffer — all four ops truly O(1)
...
:::tip[Research Question 🔍]
What is a **ring buffer** (circular buffer) and why is it preferred over a linked list for deques in practice? How does Rust's `VecDeque` implement O(1) push/pop at both ends without extra allocation?
:::
...
<div className="svg-graphic-container margin-bottom--lg">
  <img src="/img/diagrams/hash_table.svg" alt="Hash Table" style={{width:'100%',maxWidth:'720px',display:'block',margin:'1rem auto'}} />
</div>
...
:::info[Related: Sets]
A **Set** is a hash table with no values — just keys. It answers one question efficiently: *"Is this item in the collection?"* O(1) average.

Every language wraps a hash table underneath: Python `set`, JavaScript `Set`, Rust `HashSet`. Use a Set whenever you need fast membership testing or want to de-duplicate a collection.
...
<div className="svg-graphic-container margin-bottom--lg">
  <img src="/img/diagrams/bst_tree.svg" alt="Bst Tree" style={{width:'100%',maxWidth:'720px',display:'block',margin:'1rem auto'}} />
</div>
...
:::info[Real World: B-Trees]
BSTs are the **concept**. **B-Trees** are what databases and file systems actually use.

A B-Tree is a generalised BST where each node holds **multiple keys** and has **multiple children** (not just two). This design fits how storage hardware works: disks and SSDs read data in fixed-size *pages* (4–16 KB). A B-Tree node is sized to fill exactly one page, so one disk read loads a node with dozens of keys — far faster than a BST that might need one disk read per node.

| Structure | Keys per node | Used for |
|-----------|--------------|---------|
| BST | 1 | In-memory trees, conceptual learning |
| B-Tree | Many (order *m*) | Database indexes, file system metadata |
| B+ Tree | Many; data only in leaves | Most real DB indexes — leaves form a linked list enabling efficient range scans |

**You don't implement B-Trees from scratch in application code** — you use a database that manages them for you. But understanding *why* they exist explains why adding an index speeds up a query, and why indexes have a write cost.

📖 **[CMU 15-445: Database Systems (free)](https://15445.courses.cs.cmu.edu/)** — the standard deep-dive if you go into databases.
:::
...
<div className="svg-graphic-container margin-bottom--lg">
  <img src="/img/diagrams/graph_nodes.svg" alt="Graph Nodes" style={{width:'100%',maxWidth:'720px',display:'block',margin:'1rem auto'}} />
</div>
...
A **heap** is a **tree-shaped data structure** that efficiently answers one question: *"What is the smallest (or largest) item right now?"*

- **Min-heap:** the root is always the smallest element
- **Max-heap:** the root is always the largest element
- **Priority queue** is the abstract interface; a heap is the typical implementation
...
**Heap invariant:** every parent is ≤ all its children (min-heap). The tree is also *complete* — all levels are filled left-to-right. This lets you store the heap in a **plain array** with no pointers: the parent of index `i` is `(i-1)//2`, children are `2i+1` and `2i+2`.

**Real uses:** priority queues, Dijkstra's shortest path, task scheduling by priority, top-K problems (k largest elements from a stream).
...
| push (insert) | O(log n) | Bubble up to restore invariant |
| pop min/max | O(log n) | Sift down after removing root |
| heapify (build from list) | O(n) | Faster than inserting one-by-one |
| search arbitrary value | O(n) | Heaps are not sorted — use a BST for that |

**How it works in code:** You almost never implement a heap from scratch — every language provides one. The interesting part is *how* the array trick works: storing a complete binary tree in an array (no pointers needed) and maintaining the heap property by "bubbling up" after insert or "sifting down" after pop. For practical work, learn to use the language's built-in and understand when a heap beats a sorted list (answer: when you insert and delete the min frequently, rather than sorting once).

<Tabs>
<TabItem value="pseudo" label="Pseudocode">

```pseudocode
// Abstract usage — min-heap / priority queue
...
peek(heap)   // → 1  (always the minimum, O(1))
pop(heap)    // → 1  (removes and returns minimum, O(log n))
peek(heap)   // → 2  (next minimum)

// Classic use: process tasks by priority
task_queue ← NEW MinHeap<Pair<Priority, Task>>
push(task_queue, (1, "High priority task"))
push(task_queue, (5, "Low priority task"))
push(task_queue, (2, "Medium priority task"))

WHILE NOT is_empty(task_queue) DO
    (priority, task) ← pop(task_queue)   // always lowest number first
    process(task)
END WHILE
```
...
# heapq implements a MIN-heap on a plain Python list
nums = [5, 1, 3, 2, 4]
heapq.heapify(nums)             # O(n) — convert in-place

print(nums[0])                  # 1 — peek min (always index 0, O(1))
...
max_heap = [-x for x in [5, 1, 3, 2, 4]]
heapq.heapify(max_heap)
print(-heapq.heappop(max_heap)) # 5 — largest element

# Priority queue: tuples are sorted by first element
tasks = []
...
# Output: [1] urgent!, [3] medium, [5] low priority

# Top-K largest from a large stream
...
```typescript
// TypeScript has no built-in heap — minimal min-heap implementation
class MinHeap<T> {
    private data: T[] = [];
...
[5, 1, 3, 2, 4].forEach(n => heap.push(n));
console.log(heap.pop()); // 1
console.log(heap.pop()); // 2

// Priority queue with objects
const pq = new MinHeap<{ priority: number; task: string }>(
    (a, b) => a.priority - b.priority
);
pq.push({ priority: 5, task: 'low' });
pq.push({ priority: 1, task: 'urgent' });
...
for x in [5, 1, 3, 2, 4] { min_heap.push(Reverse(x)); }
println!("{:?}", min_heap.pop());   // Some(Reverse(1)) — smallest first
```
...
:::tip[Research Question 🔍]
Why is `heapify` O(n) even though inserting n items one-by-one is O(n log n)? What is the difference between a heap and a sorted array for a priority queue? When would you choose each? Look up **Dijkstra's algorithm** — how does it use a heap?
:::
...
**Combines:** Linked list, stack, queue, deque, hash table, BST  
...
- [ ] Know the Big-O of insert/delete/search for all 8 structures (can say from memory)
