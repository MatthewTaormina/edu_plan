...
```mermaid
flowchart LR
    subgraph q4["After Dequeue"]
        direction LR
        F4([FRONT]) --> D20[20] --> D30[30]
    end
    subgraph q3["Enqueue 30"]
        direction LR
        F3([FRONT]) --> C10[10] --> C20[20] --> C30[30]
    end
    subgraph q2["Enqueue 20"]
        direction LR
        F2([FRONT]) --> B10[10] --> B20[20]
    end
    subgraph q1["Enqueue 10"]
        direction LR
        F1([FRONT]) --> A10[10]
    end
    q1 --> q2 --> q3 --> q4
    note["Dequeue removes 10\nfrom FRONT → returns 10"]
    q3 --> note
```
...
```mermaid
flowchart LR
    K1["key: &quot;name&quot;"] --> H1["hash()"] --> B1["bucket[42]"] --> V1["&quot;Alice&quot;"]
    K2["key: &quot;age&quot;"]  --> H2["hash()"] --> B2["bucket[7]"]  --> V2["25"]
    K3["key: &quot;city&quot;"] --> H3["hash()"] --> B3["bucket[19]"] --> V3["&quot;Toronto&quot;"]
    style B1 fill:#4c1d95,color:#fff
    style B2 fill:#4c1d95,color:#fff
    style B3 fill:#4c1d95,color:#fff
```
