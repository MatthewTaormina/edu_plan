...
description: TypeScript type system, tsconfig, generics, utility types, declaration files, and DOM typing.
...
> **Course:** [TypeScript Fundamentals](../../courses/typescript_fundamentals/index)

<YouTubeEmbed
  id="30LWjhZzg50"
  title="TypeScript Full Course for Beginners by freeCodeCamp"
  caption="Complete TypeScript course by freeCodeCamp — covers types, interfaces, generics, and TypeScript in practice."
/>
...
## TypeScript in Different Runtimes

TypeScript compiles to plain JavaScript and runs wherever JavaScript runs:

| Runtime | Notes |
|---------|-------|
| **Browser** | Compile with `tsc` or a bundler (Vite, Webpack). Never ship `.ts` files directly. |
| **Node.js** | Use `tsx` for dev, compile to JS for production. Or `ts-node`. |
| **Deno** | Executes TypeScript natively — no compile step needed. |
...
- [TypeScript Playground](https://www.typescriptlang.org/play) — run TS in the browser, see compiled output
